...
def demo_basic_caching():
    ...
    cache_info = model_cache.get_cache_info()
    print(
        f"   📦 Models now cached: {cache_info['memory_cache_size']} in memory, {cache_info['disk_cache_size']} on disk"
    )
    ...
    try:
        images = image_generator.generate_image(
            prompt="a simple test image", fast_mode=True, num_images=1
        )
...
def demo_advanced_features():
    ...
    for i in range(5):
        ...
        print(
            f"   {elapsed:>6.1f} | {info['memory_cache_size_mb']:>9.1f} | "
...
def demo_performance_comparison():
    ...
    if no_cache_time > 0:
        ...
        cache_info = model_cache.get_cache_info()
        print(
            f"   📦 Models cached: {cache_info['memory_cache_size']} in memory, {cache_info['disk_cache_size']} on disk"
        )
