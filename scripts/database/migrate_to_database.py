...
def find_faiss_files(embeddings_dir: str = "embeddings") -> list:
    ...
    for file_path in embeddings_path.rglob("*.index"):
        ...
        if metadata_path.exists():
            faiss_files.append(
                {"index_path": str(file_path), "metadata_path": str(metadata_path), "name": file_path.stem}
            )
...
def migrate_dataset(
    faiss_index_path: str, metadata_path: str, dataset_name: str, batch_size: int = 100, dry_run: bool = False
) -> int:
    """
    ...
    if not dry_run:
        ...
        migrated_count = embedding_manager.migrate_from_faiss(
            faiss_index_path=faiss_index_path, metadata_path=metadata_path, batch_size=batch_size
        )
    else:
        # Simulate migration
        import pickle
