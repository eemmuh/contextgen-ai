...
class CacheEntry:
    ...
    def __init__(self, model: Any, model_type: str, model_name: str, 
                 device: str, parameters: Dict, cache_path: Path):
        self.model = model
...
class ModelCache:
    ...
    def __init__(self, 
        ...
        logger.info(f"ModelCache initialized: memory_limit={max_memory_size_mb}MB, "
    ...
    def _evict_if_needed(self, required_size: int = 0):
        ...
        with self._lock:
            # Check memory cache
            while (self._memory_cache_size + required_size > self.max_memory_size and 
                   self._memory_cache):
    ...
    def _evict_disk_cache(self, required_size: int):
        ...
        for cache_key, metadata in self.metadata.items():
            if 'last_accessed' in metadata:
                entries.append((cache_key, metadata['last_accessed'], metadata.get('size_bytes', 0)))
